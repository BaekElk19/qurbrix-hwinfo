...
| 摄像头 | USB/vendor 表、video source | qurbrix 使用 `v4l2-ctl --list-devices`，失败时 fallback 到 `/sys/class/video4linux/video*` 基础 name/node/driver，可从 video4linux sysfs 链接补 USB VID/PID、manufacturer/product/serial、bus/interface 和 speed 字段，并可从 `v4l2-ctl --list-formats-ext` 补格式/分辨率 capability | 厂商 alias、lshw/hwinfo enrichment 仍不足 | P2 | 是 | Kylin `.../cpuinfo.py:484-491`；qurbrix `crates/hw-probe/src/camera.rs` |

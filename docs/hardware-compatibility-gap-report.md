...
| CPU | `lscpu`、`lshw -class processor`、`dmidecode -t 4`、`/proc/cpuinfo`、`/proc/hardware`、`/sys/devices/system/cpu/cpu0/cpufreq` | 多源扫描；已补 `Hardware`/`Processor`/LoongArch `cpu model` fallback、Kirin fallback、DMI fallback、Kylin-style cpufreq 频率兜底、locale 强制、主要 vendor alias、扩展 CPU 字段和 Intel/AMD/海光/兆芯/飞腾/鲲鹏/麒麟/申威/LoongArch 等基础 CPU fixtures；ARM64 泛型 `Processor` 不再抢占后续 `Hardware` 具体型号；仍缺更广泛真机 fixture | `qurbrix-hwinfo/crates/hw-probe/src/existing.rs`；`qurbrix-hwinfo/crates/hw-parser/src/cpu.rs` |
...
| --- | --- | --- | --- | --- | --- | --- |
| CPU | 多源合并、DMI fallback、国产 vendor/arch 处理 | 已实现多源合并、`/proc/cpuinfo` fallback（含 LoongArch `cpu model`，并让后续 `Hardware` 覆盖 ARM64 泛型 `Processor`）、`/proc/hardware` Kirin fallback 和 locale 强制 | 更广泛真机样本、多源冲突样本和更多 LoongArch/SW64 变体仍不足 | P1/P2 | 是 | Deepin `.../DeviceGenerator.cpp:173-259`；qurbrix `crates/hw-probe/src/existing.rs` |

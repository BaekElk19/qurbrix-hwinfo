...
`Hardware`/`Processor` fallback、`/proc/hardware` Kirin fallback、DMI 当前频率/count 修正、Kylin-style cpufreq sysfs 频率兜底、CPU vendor/arch 归一化（含 Kylin-style D2000-only Phytium 推断），并已把 family/model/stepping/bogomips/virtualization 和 lscpu L1d/L1i/L2/L3 cache 字段暴露到 `CpuInfo`；CPU 命令源 `lscpu`、`lshw -class processor`、`dmidecode -t 4` 成功但解析不到可用 CPU 字段时会产生 `source_empty` 并继续尝试低优先级 fallback，`/proc/cpuinfo` 或 `/proc/hardware` 可读但解析为空时也会产生 `source_empty`。Memory 已在 `dmidecode -t memory` 成功路径保留 DIMM size/vendor/type/speed/slot/serial/part、data/total width 和 min/max/configured voltage，并在 DMI 不可用或解析为空时用 `lshw -class memory` 恢复 DIMM 级字段，再尝试 `decode-dimms` SPD/eeprom 文本记录，再尝试 raw DDR4/DDR5 SPD EEPROM sysfs 记录（DDR5 当前保留身份字段，并把常见 SPD manufacturer ID 映射为内存设备 vendor），再尝试 EDAC sysfs DIMM 节点，最后退回 `/proc/meminfo` 总量，空解析会产生 `source_empty` warning；System 已从 runtime 与 `dmidecode -t 1` 保留 hostname/OS/kernel/architecture 和 System Information 字段，并在 dmidecode 不可用时用 `/sys/class/dmi/id` 产品字段 fallback；BIOS/主板已在 `dmidecode -t 0,1,2,3` 不可用时读取 `/sys/class/dmi/id`，并从 `/sys/firmware/efi` 补 firmware type 和 Secure Boot state。
...
| CPU | `lscpu`、`lshw -class processor`、`dmidecode -t 4`、`/proc/cpuinfo`、`/proc/hardware`、`/sys/devices/system/cpu/cpu0/cpufreq` | 多源扫描；已补 `Hardware`/`Processor`/LoongArch `cpu model` fallback、Kirin fallback、DMI fallback、Kylin-style cpufreq 频率兜底、locale 强制、主要 vendor alias（含 D2000-only Phytium 推断）、扩展 CPU 字段、lscpu cache 字段和 Intel/AMD/海光/兆芯/飞腾/鲲鹏/麒麟/申威/LoongArch 等基础 CPU fixtures；`lscpu`/`lshw -class processor`/`dmidecode -t 4` 或可读 `/proc/cpuinfo`/`/proc/hardware` 成功但无可用 CPU 字段时会产生 `source_empty` 并继续 fallback；ARM64 泛型 `Processor` 不再抢占后续 `Hardware` 具体型号；仍缺更广泛真机 fixture | `qurbrix-hwinfo/crates/hw-probe/src/existing.rs`；`qurbrix-hwinfo/crates/hw-parser/src/cpu.rs` |
| CPU model | `CpuInfo` 有 name/vendor/arch/core/thread/socket/frequency/family/model/stepping/bogomips/virtualization/l1d_cache/l1i_cache/l2_cache/l3_cache/flags | model 已可承载并由 parser/probe 填充主要 CPU 字段 | `qurbrix-hwinfo/crates/hw-model/src/properties.rs` |
...
| Tests/fixtures | 有 PCI/USB/蓝牙/打印机/电源/音频/输入/摄像头和 CPU fixtures | CPU 已有 Intel、AMD、海光、兆芯、飞腾、鲲鹏、麒麟、申威、LoongArch 类覆盖；仍缺更多国产平台真机样本和多源冲突样本 | `qurbrix-hwinfo/crates/hw-testdata/fixtures/cpu/proc-cpuinfo-intel-x86_64.txt`；`qurbrix-hwinfo/crates/hw-testdata/fixtures/cpu/proc-cpuinfo-phytium-arm64.txt`；`qurbrix-hwinfo/crates/hw-testdata/fixtures/cpu/proc-cpuinfo-kunpeng-arm64.txt`；`qurbrix-hwinfo/crates/hw-testdata/fixtures/cpu/proc-cpuinfo-hisilicon-kirin.txt` |
...
| --- | --- | --- | --- | --- | --- |
| 通用 x86_64 | Deepin 解析 model/vendor/family/model/stepping/frequency/flags/virtualization，并在生成 CPU 概览时使用 lscpu L1/L2/L3 cache 字段；Kylin 用 lscpu 计算 core | 已扩展 lscpu/procfs/DMI 主要字段，并暴露 family/model/stepping/bogomips/virtualization 和 L1d/L1i/L2/L3 cache 字段 | 更广泛真机 fixture 仍不足 | 后续补充真实机器样本覆盖 | Deepin: `.../DeviceCpu.cpp:224-289`；Deepin: `.../DeviceGenerator.cpp:1532-1594`；qurbrix: `qurbrix-hwinfo/crates/hw-parser/src/cpu.rs` |
...
| --- | --- | --- | --- | --- | --- | --- |
| CPU | 多源合并、DMI fallback、国产 vendor/arch 处理、lscpu cache 字段 | 已实现多源合并、`/proc/cpuinfo` fallback（含 LoongArch `cpu model`，并让后续 `Hardware` 覆盖 ARM64 泛型 `Processor`）、`/proc/hardware` Kirin fallback、cpufreq sysfs fallback、lscpu L1d/L1i/L2/L3 cache 字段、D2000-only Phytium vendor 推断、命令源空解析 `source_empty` 和 locale 强制 | 更广泛真机样本、多源冲突样本和更多 LoongArch/SW64 变体仍不足 | P1/P2 | 是 | Deepin `.../DeviceGenerator.cpp:173-259`；qurbrix `crates/hw-probe/src/existing.rs` |
...
当前 fixtures 已包含 PCI/USB/蓝牙/打印机/电源/音频/输入/摄像头和 CPU 基础样本；CPU 已覆盖 Intel lscpu/procfs、AMD procfs、海光 procfs、兆芯 procfs、飞腾 ARM64 procfs、鲲鹏 ARM64 procfs、麒麟 ARM64 procfs、申威 procfs（含 `sw_64` architecture）、LoongArch procfs/lscpu 等路径，仍需更多国产平台真机输出、多源冲突输出和 SW64 变体。证据：`qurbrix-hwinfo/crates/hw-testdata/fixtures/cpu/proc-cpuinfo-intel-x86_64.txt`，`qurbrix-hwinfo/crates/hw-testdata/fixtures/cpu/proc-cpuinfo-phytium-arm64.txt`，`qurbrix-hwinfo/crates/hw-testdata/fixtures/cpu/proc-cpuinfo-kunpeng-arm64.txt`，`qurbrix-hwinfo/crates/hw-testdata/fixtures/cpu/proc-cpuinfo-hisilicon-kirin.txt`，`qurbrix-hwinfo/crates/hw-testdata/fixtures/cpu/proc-cpuinfo-sunway.txt`，`qurbrix-hwinfo/crates/hw-testdata/fixtures/cpu/proc-cpuinfo-loongarch.txt`。

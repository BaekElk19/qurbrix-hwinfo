...
补充（2026-07-06）：Monitor 已使用 `xrandr --query`、`xrandr --verbose` 和 `/sys/class/drm/*/edid`，在进程内解析 EDID vendor/product/week/year/size/preferred mode/gamma，并从物理尺寸计算 diagonal inches；剩余差距是 maxmode、`hwinfo_monitor` 和重复 sysfs connector 的精确匹配。
...
9. 优先补齐顺序建议：P1 国产 CPU/vendor/arch 真机 fixtures；P1/P2 显示器 maxmode/`hwinfo_monitor`/connector 匹配；P2 音频 SoC alias、USB/network 过滤和分类增强。
...
| Monitor | `xrandr --query` + `xrandr --verbose` + `/sys/class/drm/*/edid` | 取 connector/resolution，并从 EDID 补 vendor/product/week/year/size/preferred mode/gamma/diagonal inches；仍缺 maxmode、`hwinfo_monitor` 和重复 connector 精确匹配 | `qurbrix-hwinfo/crates/hw-probe/src/existing.rs` |
...
| Monitor | 使用 `xrandr`/verbose/EDID 处理显示器信息 | qurbrix monitor 已吸收 query/verbose/sysfs EDID 路径，剩余 maxmode、`hwinfo_monitor`、重复 connector 匹配 | `ReferenceProject/deepin-devicemanager-6.0.67/deepin-devicemanager/src/GenerateDevice/GetInfoPool.cpp:109-110` |
...
| Monitor EDID | 从 xrandr verbose 提取 EDID，用 `edid-decode` 解析 manufacturer/product/week/year/size/gamma/maxmode | qurbrix 已吸收 manufacturer/product/week/year/size/gamma，并计算 diagonal inches；maxmode 仍可参考 | `ReferenceProject/kylin-os-manager-build-2.0.0-76update2/rubbish-clear-dbus/src/detailinfo/cpuinfo.py:1339-1411` |
...
| `/sys/class/drm` | 参考项目使用 xrandr/EDID 类能力 | 通过 xrandr verbose/EDID | 已用于 monitor sysfs EDID fallback 和 GPU DRM VRAM enrichment | 已吸收部分 | 后续补重复 connector 精确匹配和更多 DRM 显示能力 |
...
| `/sys/block` | Deepin 用 lsblk/sg | Kylin 磁盘逻辑复杂 | Storage probe 已读取 vendor/model/serial/WWN/firmware/size/rotational，并从 uevent 补 kernel driver；可选 `lshw -class disk` 补人类可读 vendor/model/serial/firmware；可选 `smartctl -a -j` 补 SMART health/temperature 和 NVMe 健康计数 | 已吸收部分 | 后续补 controller/hwinfo/hdparm |
...
| `xrandr` | query/verbose | verbose + EDID | 已使用 `xrandr --query` 和 `xrandr --verbose`，并解析 verbose EDID | 已吸收部分 | 后续补 maxmode 或 `hwinfo_monitor` |
...
| 主板/BIOS/DMI | 多个 dmidecode type | dmidecode `0,1,2,3` + `/sys/class/dmi/id` fallback，并从 `/sys/firmware/efi` 补 firmware type / Secure Boot state | chassis/system/language/memory-array 未建模 | P2 | 部分 | qurbrix `crates/hw-probe/src/existing.rs` |
...
| 显示器 | xrandr verbose/EDID/product/year/size/gamma/maxmode | xrandr query/verbose + sysfs EDID，已补 vendor/product/week/year/size/preferred mode/gamma/diagonal inches | 缺 maxmode、`hwinfo_monitor` 和重复 connector 精确匹配 | P2 | 是 | Kylin `.../cpuinfo.py:1339-1411`；qurbrix `crates/hw-probe/src/existing.rs` |
...
| 项 | 内容 |
| --- | --- |
| 目标 | 显示器 EDID/DRM/sysfs 已吸收，剩余 maxmode、`hwinfo_monitor` 和重复 connector 匹配 |
| 涉及模块 | `MonitorProbe`、monitor parser、fixtures |
| 推荐实现方式 | 已用 `xrandr --verbose` 和 `/sys/class/drm/*/edid` optional source 解析 manufacturer/product/week/year/size/preferred mode/gamma，并计算 diagonal inches；后续最小增量是补 maxmode 或可选 `hwinfo_monitor` |
...
| 验收标准 | 有 EDID 时 vendor/product/size/date/gamma/diagonal inches 填充；无 EDID 不影响 resolution；后续 maxmode/`hwinfo_monitor` 有独立断言 |

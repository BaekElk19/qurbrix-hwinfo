...
| Network | `ip -j link` + `ip -j addr` + `/sys/class/net/*` enrichment/fallback | 正常路径取 interface/MAC/operstate/IPv4/IPv6，并从 sysfs 补 speed/duplex、wireless/ethernet capability、`network_type`、uevent driver、driver modules 和 PCI bus ID；fallback 路径也补 sysfs 字段；过滤 loopback/常见虚拟网卡；仍缺 lshw/NM DBus 的人类可读 product/vendor/capacity/firmware | `qurbrix-hwinfo/crates/hw-probe/src/existing.rs` |
...
| `/sys/class/net` | Deepin 有网络 sysfs/MAC 过滤逻辑 | 结合 lshw/lspci/driver | Network probe 已读取 MAC/operstate/speed/duplex、wireless/ethernet capability、`network_type`、uevent driver、PCI bus ID 和 driver modules，并在 `ip` 失败时作为 fallback；`ip -j addr` 已补 IPv4/IPv6 | 已吸收部分 | 后续补 NM DBus/lshw 人类可读 enrichment |

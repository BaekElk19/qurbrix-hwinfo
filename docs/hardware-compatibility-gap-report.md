...
| Memory | `dmidecode -t memory` + `lshw -class memory` fallback + `/proc/meminfo` fallback | 可识别 DIMM size/vendor/type/speed/slot/serial/part；DMI 不可用时先尝试 lshw DIMM bank，再退回总量 | `qurbrix-hwinfo/crates/hw-probe/src/existing.rs` |

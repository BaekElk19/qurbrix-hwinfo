...
| `/sys/class/dmi/id` | 主要走 dmidecode | 可作为系统信息来源 | System、BIOS、board、chassis probe 已作为 dmidecode fallback 使用 | 已吸收 | 后续可扩 BIOS language / Physical Memory Array |
...
| `/sys/block` | Deepin 用 lsblk/sg | Kylin 磁盘逻辑复杂 | Storage probe 已读取 vendor/model/serial/WWN/firmware/size/rotational，并从 uevent 补 kernel driver；可选 `lshw -class disk` 和 `hwinfo --disk` 补人类可读 vendor/model/serial/firmware/driver；可选 `smartctl -a -j` 补 SMART health/temperature 和 NVMe 健康计数 | 已吸收部分 | 后续补 controller/hdparm |
...
| 主板/BIOS/DMI | 多个 dmidecode type | `SystemProbe` 已读取 runtime、`dmidecode -t 1` 和 sysfs DMI 产品字段；BIOS/board/chassis 使用 dmidecode `0,1,2,3` + `/sys/class/dmi/id` fallback，并从 `/sys/firmware/efi` 补 firmware type / Secure Boot state | BIOS language / Physical Memory Array 未建模 | P2 | 部分 | qurbrix `crates/hw-probe/src/existing.rs` |
...
| 硬盘/分区/控制器 | lsblk/sg/lshw/厂商修正 | lsblk disk，失败时 fallback 到 `/sys/block/*` 并保留 vendor/model/serial/WWN/firmware/size/rotational/driver；可选 `lshw -class disk` 和 `hwinfo --disk` 补人类可读 vendor/model/serial/firmware/driver；可选 smartctl 补 SMART health/temperature 和 NVMe 健康计数并处理非零状态 JSON | 缺 controller/hdparm enrichment | P2 | 是 | qurbrix `crates/hw-probe/src/existing.rs` |

...
CPU 还补了 Kylin-style cpufreq sysfs 频率兜底、CPU vendor/arch 归一化（含 D2000-only Phytium 推断，以及 Loongson/Phytium/Hygon/Zhaoxin/Huawei/HiSilicon/Sunway 等常见 vendor/manufacturer 字符串）、Deepin-style CPU 名称尾部 `x <count>` 实例数清理、拓扑无歧义推导、placeholder 清洗、命令空解析 `source_empty` 和 procfs/sysfs fallback。Memory 已在 `dmidecode -t memory` 成功路径保留 DIMM size/vendor/type/speed/slot/serial/part、data/total width 和 min/max/configured voltage，并在 DMI 不可用或解析为空时用 `lshw -class memory` 恢复 DIMM 级字段，再尝试 `decode-dimms` SPD/eeprom 文本记录，再尝试 raw DDR4/DDR5 SPD EEPROM sysfs 记录（DDR5 当前保留身份字段，并把常见 SPD manufacturer ID 映射为内存设备 vendor），再尝试 EDAC sysfs DIMM 节点，最后退回 `/proc/meminfo` 总量，空解析会产生 `source_empty` warning；System 已从 runtime 与 `dmidecode -t 1` 保留 hostname/OS/kernel/architecture 和 System Information 字段，并在 dmidecode 不可用时用 `/sys/class/dmi/id` 产品字段 fallback；BIOS/主板已在 `dmidecode -t 0,1,2,3` 不可用时读取 `/sys/class/dmi/id`，并从 `/sys/firmware/efi` 补 firmware type 和 Secure Boot state。
...
| Memory | `dmidecode -t memory` + `lshw -class memory` fallback + optional `decode-dimms` SPD fallback + raw DDR4/DDR5 SPD EEPROM sysfs fallback + EDAC sysfs DIMM fallback + `/proc/meminfo` fallback | 可识别 DIMM size/vendor/type/speed/slot/serial/part、data/total width、min/max/configured voltage；DMI 不可用或解析为空时先尝试 lshw DIMM bank，再尝试 `decode-dimms` SPD/eeprom 文本记录，再尝试 raw DDR4 SPD EEPROM size/常见 vendor ID/type/speed/serial/part/slot，或 raw DDR5 SPD identity type/常见 module vendor/serial/part，常见 SPD ID 已覆盖 Samsung/SK Hynix/Micron/Crucial/Kingston/Ramaxel/ADATA/Longsys/Kimtigo，再尝试 EDAC sysfs DIMM label/type/size，最后退回总量；空解析会产生 warning | `qurbrix-hwinfo/crates/hw-probe/src/existing.rs` |
...
| `dmidecode` | BIOS/board/memory/CPU 修正 | 系统硬件常用来源 | CPU、BIOS/board/memory 来源；BIOS/board 权限不足走 sysfs；memory 权限不足先走 `lshw -class memory` DIMM fallback，再走 `decode-dimms` SPD/eeprom 文本 fallback，再走 raw DDR4/DDR5 SPD EEPROM sysfs fallback，再走 EDAC sysfs DIMM fallback，最后走 `/proc/meminfo` 总量 fallback | 已吸收部分 | memory 仍可补 DDR5 raw SPD 容量/速度、当前常见表之外的更完整厂商 ID 友好名称表和更细时序字段 |
...
| 内存 | dmidecode type 17 等 | dmidecode memory 成功路径保留 size/vendor/type/speed/slot/serial/part、data/total width、min/max/configured voltage；dmidecode 失败时用 `lshw -class memory` 产出 DIMM fallback，再用 `decode-dimms` 产出 SPD/eeprom DIMM vendor/serial/part/type/speed/size fallback，再用 raw DDR4 SPD EEPROM 产出 size/常见 vendor ID/type/speed/serial/part/slot fallback，raw DDR5 SPD EEPROM 产出 type/常见 module vendor/serial/part identity fallback，常见 SPD manufacturer ID 可映射 Samsung/SK Hynix/Micron/Crucial/Kingston/Ramaxel/ADATA/Longsys/Kimtigo，未知 SPD manufacturer ID 会保留为 JEP106 原始编码，再用 EDAC sysfs 产出 DIMM label/type/size fallback，最后用 `/proc/meminfo` 产出总量 fallback | 缺 DDR5 raw SPD 容量/速度、当前常见表之外的更完整厂商 ID 友好名称表和更细时序字段 | P2 | 是 | qurbrix `crates/hw-probe/src/existing.rs` |
...
| 推荐实现方式 | network 已加 `/sys/class/net` speed/duplex/wireless/ethernet capability/`network_type`/driver/modules/PCI bus ID enrichment、`ip -j addr` 地址补齐和 `lshw -class network` 人类可读 enrichment，后续补 NM DBus；USB 已补 `/sys/bus/usb/devices/*` success-path enrichment/fallback、max power 和 `lsusb -v` 首个 interface descriptor，后续补多 interface 建模；DMI 已加 `/sys/class/dmi/id` fallback 和 `/sys/firmware/efi` firmware type/Secure Boot enrichment；memory 已补 `lshw -class memory` DIMM fallback、`decode-dimms` SPD/eeprom fallback、raw DDR4 SPD EEPROM fallback、raw DDR5 SPD identity fallback、常见 SPD manufacturer ID 友好名称映射和 EDAC sysfs DIMM fallback；storage 已补 `/sys/block` rotational/WWN/firmware/vendor/driver、NVMe controller PCI identity、canonical sysfs device path PCI ancestor、有限 SATA/SCSI parent uevent PCI identity、唯一 storage-class PCI controller fallback、唯一 media-type-matching PCI storage controller fallback、`lshw -class disk` 人类可读 identity、`lshw -class storage`/`lspci -nn -k` controller identity 和 smartctl SMART/temp/NVMe 健康计数 |

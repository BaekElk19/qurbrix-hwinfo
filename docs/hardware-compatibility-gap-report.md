...
`Hardware`/`Processor` fallback、`/proc/hardware` Kirin fallback、DMI 当前频率/count 修正、CPU vendor/arch 归一化，并已把 family/model/stepping/bogomips/virtualization 暴露到 `CpuInfo`；Memory 已在 `dmidecode -t memory` 不可用或解析为空时用 `lshw -class memory` 恢复 DIMM 级字段，再退回 `/proc/meminfo` 总量，空解析会产生 `source_empty` warning；PCI 已在 `lspci -nn -k` 不可用时读取 `/sys/bus/pci/devices/*` 基础 ID 字段和 kernel driver，GPU 已可消费其中 display-class 节点并保留 driver；Network 已可从 `ip -j addr` 补 IPv4/IPv6，并可用 sysfs 标记 wireless/ethernet capability 和 `network_type`；Storage 已可从 `/sys/block/*/device/uevent` 补 kernel driver，并可从 `smartctl -a -j` 补 SMART health/temperature（含非零状态但 JSON 可解析的输出）；USB 已可用 `/sys/bus/usb/devices/*` enrich `lsusb` 成功路径，并在 `lsusb` 不可用时读取 sysfs 基础 device 字段和 max power；Audio 已在 `/proc/asound/cards` 不可用时读取 `/sys/class/sound/card*` 基础声卡节点，并从 `/proc/asound/card*/codec#*` 与 sysfs 声卡节点补 codec/driver/vendor/subsystem；Bluetooth 已在 `hciconfig -a` 不可用时读取 `/sys/class/bluetooth/hci*` controller address 和 rfkill 字段；Input 已在 `/proc/bus/input/devices` 不可用时读取 `/sys/class/input/event*` 基础事件节点，并可用 procfs/sysfs evdev capability bitmask 分类 touchscreen/touchpad/tablet/mouse；Battery 已可在 UPower/sysfs 路径归一化 `LGC` 为 `LG Chem`，并可从 sysfs fallback 补电池温度；Camera 已在 `v4l2-ctl --list-devices` 不可用时读取 `/sys/class/video4linux/video*` 基础节点，并从 video4linux sysfs 补 kernel driver 和 USB identity；Printer 已在 `lpstat -a` 不可用时使用 `lpstat -v` 恢复基础队列/URI，从 `lpstat -d` 标记默认队列，并从 `lpstat -l -p` 补 make/model 描述；CD-ROM 已可用 `/sys/class/block/sr*` enrich proc 成功路径，并在 `/proc/sys/dev/cdrom/info` 不可用时读取基础光驱节点和 vendor/model/serial/firmware。
...
| USB | `lsusb` + `/sys/bus/usb/devices/*` enrichment/fallback | 能解析基础 USB 字段；`lsusb` 成功时可按 bus/dev 从 sysfs 补 class/subclass/protocol、manufacturer/serial/speed/max power；无 `lsusb` 时可从 sysfs 读取 bus/dev、VID/PID、device class/subclass/protocol、manufacturer/product/serial/speed/max power；仍无 `lsusb -v`、详细 interface descriptor、跨类别 consumed dedup | `qurbrix-hwinfo/crates/hw-probe/src/usb.rs` |
| Memory | `dmidecode -t memory` + `lshw -class memory` fallback + `/proc/meminfo` fallback | 可识别 DIMM size/vendor/type/speed/slot/serial/part；DMI 不可用或解析为空时先尝试 lshw DIMM bank，再退回总量；空解析会产生 warning | `qurbrix-hwinfo/crates/hw-probe/src/existing.rs` |
...
| Storage | `lsblk -J -b -o NAME,TYPE,SIZE,MODEL,SERIAL,TRAN,WWN,REV` + `/sys/block/*` fallback + optional `smartctl -a -j` | 正常路径取 disk 并保留 WWN/firmware/kernel driver；fallback 路径补 node/vendor/model/serial/WWN/firmware/size/rotational/driver；smartctl 可补 SMART health/temperature，且保留非零状态 JSON | `qurbrix-hwinfo/crates/hw-probe/src/existing.rs` |
| GPU | `lspci -nn -k`，GPU parser；`lspci` 不可用时可消费 `/sys/bus/pci/devices/*` display-class 节点 | 能识别 PCI GPU 和 driver；sysfs fallback 可保留基础 PCI ID/VID/PID/class/driver，但仍缺 glxinfo/drm enrichment、sysfs modules 和人类可读设备名 | `qurbrix-hwinfo/crates/hw-probe/src/existing.rs` |
...
| Network | `ip -j link` + `ip -j addr` + `/sys/class/net/*` enrichment/fallback | 正常路径取 interface/MAC/operstate/IPv4/IPv6，并从 sysfs 补 speed/duplex、wireless/ethernet capability、`network_type`、uevent driver；fallback 路径也补 sysfs 字段；过滤 loopback/常见虚拟网卡；仍缺 lshw/lspci/NM DBus | `qurbrix-hwinfo/crates/hw-probe/src/existing.rs` |
...
| `/sys/class/net` | Deepin 有网络 sysfs/MAC 过滤逻辑 | 结合 lshw/lspci/driver | Network probe 已读取 MAC/operstate/speed/duplex、wireless/ethernet capability、`network_type` 和 uevent driver，并在 `ip` 失败时作为 fallback；`ip -j addr` 已补 IPv4/IPv6 | 已吸收部分 | 后续补 NM DBus/lshw/lspci |
...
| `/sys/block` | Deepin 用 lsblk/sg | Kylin 磁盘逻辑复杂 | Storage probe 已读取 vendor/model/serial/WWN/firmware/size/rotational，并从 uevent 补 kernel driver；可选 `smartctl -a -j` 补 SMART health/temperature | 已吸收部分 | 后续补 controller/lshw/hdparm |
| `/sys/bus/pci` | 参考项目重视 PCI/driver | lspci/lshw/driver | `lspci -nn -k`；无 lspci 时读 sysfs vendor/device/class/subsystem/driver，GPU 消费 display-class 节点 | P2 | 后续补 sysfs modules、人类可读名称和跨类别消费 |
| `/sys/bus/usb` | Deepin USB 过滤/去重 | Kylin `lsusb -v` | USB probe 已用 sysfs enrich `lsusb` 成功路径，并在 `lsusb` 失败时读取基础 sysfs device 字段和 max power | 已吸收部分 | 后续补 `lsusb -v`、interface descriptor、跨类别 dedup |
...
| `dmidecode` | BIOS/board/memory/CPU 修正 | 系统硬件常用来源 | CPU、BIOS/board/memory 来源；BIOS/board 权限不足走 sysfs；memory 权限不足先走 `lshw -class memory` DIMM fallback，再走 `/proc/meminfo` 总量 fallback | 已吸收部分 | memory 仍可补 sysfs DIMM 级 fallback |
| `lshw` | CPU/audio/network 等多类 fallback | 网络/硬件详情 | CPU 和 Memory 已作为 optional fallback 使用 | 已吸收部分 | 可选，不作为硬依赖，后续补 audio/network/storage/GPU 等轻量 enrichment |
...
| 内存 | dmidecode type 17 等 | dmidecode memory，dmidecode 失败时用 `lshw -class memory` 产出 DIMM fallback，再用 `/proc/meminfo` 产出总量 fallback | 缺 sysfs DIMM 级 fallback | P2 | 是 | qurbrix `crates/hw-probe/src/existing.rs` |
| 硬盘/分区/控制器 | lsblk/sg/lshw/厂商修正 | lsblk disk，失败时 fallback 到 `/sys/block/*` 并保留 vendor/model/serial/WWN/firmware/size/rotational/driver；可选 smartctl 补 SMART health/temperature 并处理非零状态 JSON | 缺 controller、lshw/hdparm enrichment | P2 | 是 | qurbrix `crates/hw-probe/src/existing.rs` |
| 显卡/显示控制器 | lspci、nvidia、国产 GPU alias | lspci GPU/driver；lspci 不可用时从 sysfs PCI display-class 生成基础 GPU 并消费对应 PCI，保留 kernel driver | 缺 glxinfo/drm、sysfs modules、人类可读设备名 | P2 | 是 | Kylin `.../cpuinfo.py:415-425`；qurbrix `crates/hw-probe/src/existing.rs` |
...
| 网卡/Wi-Fi/蓝牙 | sysfs/MAC/filter、lshw/lspci/driver | network 使用 `ip -j link`/`ip -j addr` 并用 `/sys/class/net/*` enrich speed/duplex/wireless/ethernet capability/`network_type`/driver，失败时 fallback 到 sysfs；bluetooth 使用 `hciconfig -a`，失败时 fallback 到 `/sys/class/bluetooth/hci*` controller address/rfkill 字段 | Network 仍缺 NM DBus/lshw/lspci；Bluetooth 仍缺 lshw/hwinfo/BlueZ DBus enrichment 和 paired-device fallback | P1/P2 | 是 | qurbrix `crates/hw-probe/src/existing.rs`；`crates/hw-probe/src/bluetooth.rs` |
| 声卡 | lshw、`/proc/asound`、SoC vendor | qurbrix 使用 `/proc/asound/cards`，可从 `/proc/asound/card*/codec#*` 和 `/sys/class/sound/card*/device` 补 codec/driver/vendor/subsystem，proc cards 缺失时 fallback 到 sysfs card | 仍缺 lshw/hwinfo/PCI fusion、完整 profile 和更广 SoC alias | P2 | 是 | Deepin `.../GetInfoPool.cpp:88,119`；Kylin `.../cpuinfo.py:479-483` |
| USB 设备 | 过滤 hub/重复/无效设备，详细描述符 | `lsusb` 基础字段；成功路径和 fallback 均可读 sysfs 基础字段和 max power，并过滤 hub/host controller/interface entries | 无 `lsusb -v`、详细 interface descriptor、跨类别 consumed dedup | P2 | 是 | qurbrix `crates/hw-probe/src/usb.rs` |
| PCI 设备 | PCI 分类、驱动识别 | lspci class/vendor/device/driver；lspci 缺失时 sysfs fallback 保留 IDs 和 driver | 分类消费只对部分类别；alias 和 sysfs modules 不足 | P2 | 是 | qurbrix `crates/hw-probe/src/pci.rs:22-83` |
| 摄像头 | USB/vendor 表、video source | qurbrix 使用 `v4l2-ctl --list-devices`，失败时 fallback 到 `/sys/class/video4linux/video*` 基础 name/node/driver，并可从 video4linux sysfs 链接补 USB VID/PID、manufacturer/product/serial 和 bus/interface 字段 | speed/profile、厂商 alias、lshw/hwinfo enrichment 仍不足 | P2 | 是 | Kylin `.../cpuinfo.py:484-491`；qurbrix `crates/hw-probe/src/camera.rs` |
...
| 输入设备 | `/proc/bus/input/devices` + `/sys/class/input/event*` fallback | qurbrix 有 input fixtures，并可从 procfs/sysfs evdev capability bitmask 补 touchscreen/touchpad/tablet/mouse 分类 | vendor alias、lshw/hwinfo enrichment 和更细 bus-specific 分类仍可增强 | P2 | 是 | Deepin `.../GetInfoPool.cpp:118`；Kylin `.../cpuinfo.py:492-503` |
...
| 推荐实现方式 | network 已加 `/sys/class/net` speed/duplex/wireless/ethernet capability/`network_type`/driver enrichment 和 `ip -j addr` 地址补齐，后续补 DBus/lshw/lspci；USB 已补 `/sys/bus/usb/devices/*` success-path enrichment/fallback 和 max power，后续加 `lsusb -v` optional interface descriptors；DMI 加 `/sys/class/dmi/id` fallback；memory 已补 `lshw -class memory` DIMM fallback；storage 已补 `/sys/block` rotational/WWN/firmware/vendor/driver 和 smartctl SMART/temp |

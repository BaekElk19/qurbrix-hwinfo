...
| 内存 | dmidecode type 17 等 | dmidecode memory，dmidecode 失败时用 `lshw -class memory` 产出 DIMM fallback，再用 `decode-dimms` 产出 SPD/eeprom DIMM vendor/serial/part/type/speed/size fallback，再用 raw DDR4 SPD EEPROM 产出 size/常见 vendor ID/type/speed/serial/part/slot fallback，raw DDR5 SPD EEPROM 产出 type/常见 module vendor/serial/part identity fallback，再用 EDAC sysfs 产出 DIMM label/type/size fallback，最后用 `/proc/meminfo` 产出总量 fallback | 缺 DDR5 raw SPD 容量/速度、完整厂商 ID 表和更细时序字段 | P2 | 是 | qurbrix `crates/hw-probe/src/existing.rs` |

...
| 内存 | dmidecode type 17 等 | dmidecode memory，dmidecode 失败时用 `lshw -class memory` 产出 DIMM fallback，再用 `decode-dimms` 产出 SPD/eeprom DIMM vendor/serial/part/type/speed/size fallback，再用 raw DDR4 SPD EEPROM 产出 size/type/speed/serial/part/slot fallback，再用 EDAC sysfs 产出 DIMM label/type/size fallback，最后用 `/proc/meminfo` 产出总量 fallback | 缺 DDR5 raw SPD 和更细时序/厂商 ID 字段 | P2 | 是 | qurbrix `crates/hw-probe/src/existing.rs` |

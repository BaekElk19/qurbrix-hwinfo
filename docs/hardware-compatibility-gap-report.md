...
| CPU | `lscpu`、`lshw -class processor`、`dmidecode -t 4`、`/proc/cpuinfo`、`/proc/hardware`、`/sys/devices/system/cpu/cpu0/cpufreq` | 多源扫描；已补 `Hardware`/`Processor`/LoongArch `cpu model` fallback、Kirin fallback、DMI fallback、Kylin-style cpufreq 频率兜底、locale 强制、主要 vendor alias、扩展 CPU 字段、lscpu cache 字段和 Intel/AMD/海光/兆芯/飞腾/鲲鹏/麒麟/申威/LoongArch 等基础 CPU fixtures；ARM64 泛型 `Processor` 不再抢占后续 `Hardware` 具体型号；仍缺更广泛真机 fixture | `qurbrix-hwinfo/crates/hw-probe/src/existing.rs`；`qurbrix-hwinfo/crates/hw-parser/src/cpu.rs` |
...
| --- | --- | --- | --- | --- | --- | --- |
| CPU | 多源合并、DMI fallback、国产 vendor/arch 处理、lscpu cache 字段 | 已实现多源合并、`/proc/cpuinfo` fallback（含 LoongArch `cpu model`，并让后续 `Hardware` 覆盖 ARM64 泛型 `Processor`）、`/proc/hardware` Kirin fallback、cpufreq sysfs fallback、lscpu L1d/L1i/L2/L3 cache 字段和 locale 强制 | 更广泛真机样本、多源冲突样本和更多 LoongArch/SW64 变体仍不足 | P1/P2 | 是 | Deepin `.../DeviceGenerator.cpp:173-259`；qurbrix `crates/hw-probe/src/existing.rs` |
...
| 内存 | dmidecode type 17 等 | dmidecode memory 成功路径保留 size/vendor/type/speed/slot/serial/part、data/total width、min/max/configured voltage；dmidecode 失败时用 `lshw -class memory` 产出 DIMM fallback，再用 `decode-dimms` 产出 SPD/eeprom DIMM vendor/serial/part/type/speed/size fallback，再用 raw DDR4 SPD EEPROM 产出 size/常见 vendor ID/type/speed/serial/part/slot fallback，raw DDR5 SPD EEPROM 产出 type/常见 module vendor/serial/part identity fallback，未知 SPD manufacturer ID 会保留为 JEP106 原始编码，再用 EDAC sysfs 产出 DIMM label/type/size fallback，最后用 `/proc/meminfo` 产出总量 fallback | 缺 DDR5 raw SPD 容量/速度、更完整厂商 ID 友好名称表和更细时序字段 | P2 | 是 | qurbrix `crates/hw-probe/src/existing.rs` |
| 硬盘/分区/控制器 | lsblk/sg/lshw/厂商修正 | lsblk disk，失败时 fallback 到 `/sys/block/*` 并保留 vendor/model/serial/WWN/firmware/size/rotational/driver 和可用 PCI controller identity；NVMe namespace 可从 controller sysfs uevent 补 PCI bus identity；SATA/SCSI disk 可用 canonical sysfs device path 提取最近 PCI ancestor，可有限向父级 uevent 回溯补 PCI controller identity，或在 parent uevent 不可用时用唯一 storage-class PCI controller 或唯一 media-type-matching PCI storage controller 安全补 controller identity；可选 `lshw -class disk`、`hwinfo --disk` 和 `hdparm -i` 补人类可读 disk vendor/model/serial/firmware/driver 或 ATA identity；可选 `lshw -class storage` 或 `lspci -nn -k` 按 PCI 地址补 controller vendor/model/driver；可选 smartctl 补 SMART health/temperature 和 NVMe 健康计数并处理非零状态 JSON | 缺 sysfs 无法暴露可读 PCI ancestor 时的同类多控制器归属 | P2 | 是 | qurbrix `crates/hw-probe/src/existing.rs` |
| 显卡/显示控制器 | lspci、lshw display、dmesg、gpu-info、glxinfo、nvidia、国产 GPU alias | lspci GPU/driver/modules；可选 `lshw -class display` 按 PCI bus 补人类可读 product/vendor/driver；可选 DRM sysfs、`dmesg`、Deepin/Jingjia vendor path、`nvidia-smi` 和唯一 NVIDIA GPU 场景的 `nvidia-settings` 补 VRAM total；单显卡、多卡唯一 vendor 匹配或同 vendor 唯一 renderer/model 匹配时可选 `glxinfo -B` 补 OpenGL renderer/vendor/version；lspci 不可用时从 sysfs PCI display-class 生成基础 GPU 并消费对应 PCI，保留 kernel driver/modules | 缺 renderer 无法唯一识别型号的多 GPU glxinfo 精确归属 | P3 | 是 | Kylin `.../cpuinfo.py:415-425`；Deepin `.../commontools.cpp:333-351`；Deepin `.../DeviceGpu.cpp:196-252`；qurbrix `crates/hw-probe/src/existing.rs` |
| 显示器 | xrandr verbose/EDID/product/year/size/gamma/maxmode，service-support 使用 `hwinfo --monitor` | xrandr query/verbose + sysfs EDID + optional `hwinfo --monitor`，已补 vendor/product/week/year/size/preferred mode/gamma/diagonal inches/max resolution，hwinfo 可补 vendor/model/serial/size 或生成 fallback 设备，重复 sysfs connector 中只有一个可读 EDID、多个可读但只有一个 `status=connected`、status 不唯一但只有一个 `enabled=enabled`、或只有一个 EDID preferred mode 匹配 xrandr 当前分辨率时可使用该候选 | 缺多个可读且这些信号都无法唯一判定的重复 connector 精确归属 | P2 | 是 | Kylin `.../cpuinfo.py:1339-1411`；Kylin `hardware-capability-scan-report.md:135`；qurbrix `crates/hw-probe/src/existing.rs` |
...
| 推荐实现方式 | network 已加 `/sys/class/net` speed/duplex/wireless/ethernet capability/`network_type`/driver/modules/PCI bus ID enrichment、`ip -j addr` 地址补齐和 `lshw -class network` 人类可读 enrichment，后续补 NM DBus；USB 已补 `/sys/bus/usb/devices/*` success-path enrichment/fallback、max power 和 `lsusb -v` 首个 interface descriptor，后续补多 interface 建模；DMI 已加 `/sys/class/dmi/id` fallback 和 `/sys/firmware/efi` firmware type/Secure Boot enrichment；memory 已补 `lshw -class memory` DIMM fallback、`decode-dimms` SPD/eeprom fallback、raw DDR4 SPD EEPROM fallback、raw DDR5 SPD identity fallback 和 EDAC sysfs DIMM fallback；storage 已补 `/sys/block` rotational/WWN/firmware/vendor/driver、NVMe controller PCI identity、canonical sysfs device path PCI ancestor、有限 SATA/SCSI parent uevent PCI identity、唯一 storage-class PCI controller fallback、唯一 media-type-matching PCI storage controller fallback、`lshw -class disk` 人类可读 identity、`lshw -class storage`/`lspci -nn -k` controller identity 和 smartctl SMART/temp/NVMe 健康计数 |

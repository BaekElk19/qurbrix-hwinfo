...
| `lshw` | CPU/audio/network 等多类 fallback | 网络/硬件详情 | CPU 和 Memory 已作为 optional fallback 使用；Network、Audio、Storage 已作为 optional enrichment 使用 | 已吸收部分 | 可选，不作为硬依赖，后续补 GPU 等轻量 enrichment |

...
| GPU | `lspci -nn -k`，GPU parser；`lspci` 不可用时可消费 `/sys/bus/pci/devices/*` display-class 节点 | 能识别 PCI GPU 和 driver/modules；sysfs fallback 可保留基础 PCI ID/VID/PID/class/driver/modules，但仍缺 glxinfo/drm enrichment 和人类可读设备名 | `qurbrix-hwinfo/crates/hw-probe/src/existing.rs` |
...
| `/sys/bus/pci` | 参考项目重视 PCI/driver | lspci/lshw/driver | `lspci -nn -k`；无 lspci 时读 sysfs vendor/device/class/subsystem/driver/modules，GPU 消费 display-class 节点 | 已吸收部分 | 后续补人类可读名称和跨类别消费 |
...
| 显卡/显示控制器 | lspci、nvidia、国产 GPU alias | lspci GPU/driver/modules；lspci 不可用时从 sysfs PCI display-class 生成基础 GPU 并消费对应 PCI，保留 kernel driver/modules | 缺 glxinfo/drm、人类可读设备名 | P2 | 是 | Kylin `.../cpuinfo.py:415-425`；qurbrix `crates/hw-probe/src/existing.rs` |

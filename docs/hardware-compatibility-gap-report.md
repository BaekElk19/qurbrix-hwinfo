...
`Hardware`/`Processor` fallback、`/proc/hardware` Kirin fallback、DMI 当前频率/count 修正、CPU vendor/arch 归一化；PCI 已在 `lspci -nn -k` 不可用时读取 `/sys/bus/pci/devices/*` 基础 ID 字段和 kernel driver，GPU 已可消费其中 display-class 节点并保留 driver；Network 已可从 `ip -j addr` 补 IPv4/IPv6，并可用 sysfs 标记 wireless/ethernet capability；Storage 已可从 `/sys/block/*/device/uevent` 补 kernel driver，并可从 `smartctl -a -j` 补 SMART health/temperature（含非零状态但 JSON 可解析的输出）；USB 已可用 `/sys/bus/usb/devices/*` enrich `lsusb` 成功路径，并在 `lsusb` 不可用时读取 sysfs 基础 device 字段；Audio 已在 `/proc/asound/cards` 不可用时读取 `/sys/class/sound/card*` 基础声卡节点，并从 `/proc/asound/card*/codec#*` 与 sysfs 声卡节点补 codec/driver/vendor/subsystem；Bluetooth 已在 `hciconfig -a` 不可用时读取 `/sys/class/bluetooth/hci*` controller address 和 rfkill 字段；Input 已在 `/proc/bus/input/devices` 不可用时读取 `/sys/class/input/event*` 基础事件节点，并可用 procfs/sysfs evdev capability bitmask 分类 touchscreen/touchpad/tablet/mouse；Battery 已可在 UPower/sysfs 路径归一化 `LGC` 为 `LG Chem`；Camera 已在 `v4l2-ctl --list-devices` 不可用时读取 `/sys/class/video4linux/video*` 基础节点，并从 video4linux sysfs 补 kernel driver 和 USB identity；Printer 已在 `lpstat -a` 不可用时使用 `lpstat -v` 恢复基础队列/URI，从 `lpstat -d` 标记默认队列，并从 `lpstat -l -p` 补 make/model 描述；CD-ROM 已可用 `/sys/class/block/sr*` enrich proc 成功路径，并在 `/proc/sys/dev/cdrom/info` 不可用时读取基础光驱节点和 vendor/model/serial。
...
| Network | `ip -j link` + `ip -j addr` + `/sys/class/net/*` enrichment/fallback | 正常路径取 interface/MAC/operstate/IPv4/IPv6，并从 sysfs 补 speed/duplex、wireless/ethernet capability、uevent driver；fallback 路径也补 sysfs 字段；过滤 loopback/常见虚拟网卡；仍缺 lshw/lspci/NM DBus 和独立以太/无线 type 字段 | `qurbrix-hwinfo/crates/hw-probe/src/existing.rs` |
...
| `/sys/class/net` | Deepin 有网络 sysfs/MAC 过滤逻辑 | 结合 lshw/lspci/driver | Network probe 已读取 MAC/operstate/speed/duplex、wireless/ethernet capability 和 uevent driver，并在 `ip` 失败时作为 fallback；`ip -j addr` 已补 IPv4/IPv6 | 已吸收部分 | 后续补 NM DBus/lshw/lspci、独立 type 字段 |
| `/sys/class/power_supply` | Deepin 使用 upower/dmesg 类电源源 | Kylin 有电源厂商 alias | Battery probe 已在 UPower 失败时读取 BAT* sysfs 字段，并对 `LGC` 做轻量厂商归一化 | 已吸收部分 | 后续可补温度和更多厂商 alias |
...
| `/sys/bus/usb` | Deepin USB 过滤/去重 | Kylin `lsusb -v` | USB probe 已用 sysfs enrich `lsusb` 成功路径，并在 `lsusb` 失败时读取基础 sysfs device 字段 | 已吸收部分 | 后续补 `lsusb -v`、maxpower、interface descriptor、跨类别 dedup |
...
| 网卡/Wi-Fi/蓝牙 | sysfs/MAC/filter、lshw/lspci/driver | network 使用 `ip -j link`/`ip -j addr` 并用 `/sys/class/net/*` enrich speed/duplex/wireless/ethernet capability/driver，失败时 fallback 到 sysfs；bluetooth 使用 `hciconfig -a`，失败时 fallback 到 `/sys/class/bluetooth/hci*` controller address/rfkill 字段 | Network 仍缺 NM DBus/lshw/lspci 和独立 type 字段；Bluetooth 仍缺 lshw/hwinfo/BlueZ DBus enrichment 和 paired-device fallback | P1/P2 | 是 | qurbrix `crates/hw-probe/src/existing.rs`；`crates/hw-probe/src/bluetooth.rs` |
| 声卡 | lshw、`/proc/asound`、SoC vendor | qurbrix 使用 `/proc/asound/cards`，可从 `/proc/asound/card*/codec#*` 和 `/sys/class/sound/card*/device` 补 codec/driver/subsystem，proc cards 缺失时 fallback 到 sysfs card | 仍缺 lshw/hwinfo/PCI fusion、vendor normalization、完整 profile | P2 | 是 | Deepin `.../GetInfoPool.cpp:88,119`；Kylin `.../cpuinfo.py:479-483` |
| USB 设备 | 过滤 hub/重复/无效设备，详细描述符 | `lsusb` 基础字段；成功路径和 fallback 均可读 sysfs 基础字段并过滤 hub/host controller/interface entries | 无 `lsusb -v`、maxpower、详细 interface descriptor、跨类别 consumed dedup | P2 | 是 | qurbrix `crates/hw-probe/src/usb.rs` |
...
| 电池/电源 | upower/dmesg/vendor alias | qurbrix 有 UPower 和 `/sys/class/power_supply/BAT*` fallback fixtures，并归一化常见 `LGC` alias | 温度 fallback 和更多厂商 alias 待增强 | P2 | 是 | Deepin `.../GetInfoPool.cpp:105,111`；Kylin `.../cpuinfo.py:524-525` |
...
| 推荐实现方式 | network 已加 `/sys/class/net` speed/duplex/wireless/ethernet capability/driver enrichment 和 `ip -j addr` 地址补齐，后续补 DBus/lshw/lspci 和独立 type；USB 已补 `/sys/bus/usb/devices/*` success-path enrichment/fallback，后续加 `lsusb -v` optional interface/maxpower；DMI 加 `/sys/class/dmi/id` fallback；storage 已补 `/sys/block` rotational/WWN/firmware/vendor/driver 和 smartctl SMART/temp |

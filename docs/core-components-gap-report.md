...
| `/proc/bootdevice/cid` + `/sys/.../unique_number` | Y (`getSerialID`, `DeviceStorage.cpp:276-326`) | N | **N** |
| `/sys/block/*/device/spec_version`（UFS） | Y (`:252-261`) | N | **N** |
| `/sys/block/*/device/queue/rotational` | 部分 | N | Y |

### 4.2 字段差距

qurbrix `StorageInfo`（`properties.rs:142-162`）已覆盖 device_node/size_bytes/media_type/controller_vendor/
controller_model/controller_driver/firmware/wwn/smart_status/temperature_celsius/power_on_hours/
...
- **UFS 接口检测**（`spec_version`）
- **SATA Version → Speed 字段**（Deepin lshw `configuration.speed`）
- **RotationRate 显式字段**（当前只转 media_type，丢了 RPM/"Solid State Device" 文本）
- **Capabilities / VID_PID / PhysID / Modalias**（Deepin 全采集）
- **`checkDiskSize` 定制机型容量归一**（`DeviceStorage.cpp:529-559`，511-513GB→"512 GB"）
- **嵌入式 Serial 兜底**：`/proc/bootdevice/cid`、`/sys/.../unique_number`（eMMC/UFS 必需）

### 4.3 国产品牌前缀

qurbrix `storage_vendor_from_model_prefix`（`existing.rs:2404-2428`）已含：HGST HUS / WDC / HITACHI / HTS /
IC / FUJITSU / MP / TOSHIBA / MK / MAXTOR / PIONEER / PHILIPS / QUANTUM / FIREBALL / **FORESEE** / IBM /
**RS→Longsys** / **ST→Seagate**。

未覆盖：**YMTC / ZHITAI / ZTC / YEESTOR / MAXIO / GLOWAY / KingSpec / KINGSTON / SanDisk / SAMSUNG /
MICRON / CT (Crucial) / SK / SKHynix / HYNIX / NETAC / RAMAXEL / BIWIN / CXMT / TIGO / Colorful / Asgard / LEXAR**。
...
- **P0**：扩充 vendor 前缀表（YMTC/ZhiTai/YEESTOR/MAXIO/GLOWAY/KingSpec/KINGSTON/SanDisk/SAMSUNG/
  MICRON/Crucial/SK Hynix/NETAC/RAMAXEL/BIWIN/CXMT）；UFS `spec_version` 检测；RotationRate / Speed 字段
- **P1**：`/proc/bootdevice/cid` + `unique_number` serial 兜底；PCI/USB VID_PID + Modalias；Capabilities 文本
- **P2**：`checkDiskSize` 归一（512 GB/1 TB）；NVMe controller 与 disk 显式父子链接；USB 桥接盘 `smartctl -d sat` 重试

### 4.5 风险

- 前缀表 first-match-wins：`ST` (Seagate) 会误吞 `STEC`、`Stellar` 等，建议加最短长度或分隔符校验
- `aply_storage_smartctl` 在 JSON 解析失败时静默丢弃（`existing.rs:2190-2191`），建议加 warning
- `device_id` 优先级 `wwn > serial > node`，与 Deepin 复合 key 不对齐；USB 盘 serial 缺失时跨扫描去重会抖动
- FORESEE vs PCI vendor Longsys 冲突：Kylin `0049-73.patch:5756` 显式注释掉 FORESEE→Foresee，改用 PCI vendor
  `1d97 Longsys`。qurbrix 需决定当 model 前缀 FORESEE 但 PCI vendor Longsys 时最终展示哪个

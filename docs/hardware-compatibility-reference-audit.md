...
- Audio probing preserves PCI bus identity and kernel module names from `/sys/class/sound/card*/device/uevent` and `/driver/module/drivers/*` when available, optionally uses `lshw -class multimedia` for human-readable audio product/vendor data, and optionally uses `pactl list cards` for ALSA card profile names.
...
- Input, printer, and CD-ROM still have limited fallback/enrichment coverage compared with the reference projects; audio still lacks hwinfo enrichment; camera still lacks lshw/hwinfo enrichment.
...
- P2: add network NM DBus enrichment, storage controller/hwinfo/hdparm enrichment, richer multi-interface USB modeling, camera lshw/hwinfo enrichment, audio hwinfo enrichment, and Bluetooth hwinfo/DBus enrichments.

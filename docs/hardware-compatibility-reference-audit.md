...
- CD-ROM probing enriches `/proc/sys/dev/cdrom/info` drives from `/sys/class/block/sr*` and optional `lshw -class disk` `*-cdrom` records with basic vendor/model/serial/firmware identity, and falls back to sysfs drives when proc cdrom info cannot be read or parses no drives.
...
| CD-ROM | `/proc/sys/dev/cdrom/info`, drive names and basic capabilities; enriches proc-discovered drives from `/sys/class/block/sr*` and matched `lshw -class disk` `*-cdrom` records with vendor/model/serial/firmware identity; falls back to sysfs drive nodes plus available lshw/sysfs identity when proc cdrom info is unavailable or parses empty. | Proc cdrom discovery, Linux sysfs block enrichment/fallback, lshw CD-ROM identity enrichment, basic identity and firmware preservation, and skip-empty warning behavior. | No hwinfo/lsscsi fallback; sysfs/lshw fallback cannot recover proc cdrom capabilities. | P2 |
...
- CD-ROM preserves the missing/failed `/proc/sys/dev/cdrom/info` warning while still emitting optical drive nodes and available identity fields from `/sys/class/block/sr*`/`lshw -class disk` when present, and emits `source_empty` plus fallback when proc cdrom parsing yields no drives.

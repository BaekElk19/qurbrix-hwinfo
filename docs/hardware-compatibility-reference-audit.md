...
- Network probing uses `ip -j link` plus optional `ip -j addr`, enriches interfaces from `/sys/class/net/*` with speed, duplex, wireless/ethernet type/capability marking, kernel driver/modules, and PCI bus identity from uevent when available, and falls back to sysfs interfaces when `ip -j link` cannot run.
...
- Audio probing preserves PCI bus identity and kernel module names from `/sys/class/sound/card*/device/uevent` and `/driver/module/drivers/*` when available.
...
| Network | `ip -j link`, interface/MAC/operstate, optional `ip -j addr` IPv4/IPv6 enrichment; filters loopback/common virtual interfaces; malformed JSON produces warning; enriches interfaces from `/sys/class/net/*` with speed, duplex, wireless/ethernet `network_type` and capabilities, `DRIVER=`, PCI bus IDs, and module names from uevent/sysfs; falls back to sysfs interfaces when `ip` cannot run. | Basic network interface enumeration, IP address enrichment, Kylin-style avoidance of non-physical interfaces, Linux sysfs fallback, lightweight sysfs driver/module preservation, wireless/ethernet type/capability enrichment, and PCI bus identity preservation. | No lshw/NM DBus human-readable product/vendor/capacity/firmware enrichment. | P1/P2 |
| Audio | `/proc/asound/cards`, card index/name; enriches from `/proc/asound/card*/codec#*` for codec and `/sys/class/sound/card*/device` for driver/modules, normalized vendor, subsystem IDs, and PCI bus identity; falls back to `/sys/class/sound/card*` for ALSA card index/name plus available enrichment when proc asound cards is unavailable. | Deepin/Kylin use `/proc/asound` and multimedia sources; base source, Linux sysfs card fallback, lightweight codec, driver/module, vendor/subsystem, and PCI identity enrichment absorbed. | No lshw/hwinfo human-readable product fusion; limited profile data; vendor alias table remains intentionally small. | P2 |
...
- Input, printer, and CD-ROM still have limited fallback/enrichment coverage compared with the reference projects; audio still lacks lshw/hwinfo human-readable product fusion; camera still lacks lshw/hwinfo and speed/profile enrichment.
...
- P2: add network DBus/lshw human-readable enrichment, storage controller/lshw/hdparm enrichment, USB verbose descriptors, camera lshw/hwinfo enrichment, audio lshw/hwinfo/PCI fusion, and Bluetooth lshw/DBus enrichments.

...
- Camera probing falls back to `/sys/class/video4linux/video*` when `v4l2-ctl --list-devices` cannot run, enriches cameras with video4linux sysfs driver and USB identity data when available, and optionally preserves `v4l2-ctl --list-formats-ext` format/resolution capabilities.
...
| Camera | `v4l2-ctl --list-devices`, emits one device per physical camera record using the first `/dev/video*` node; optionally reads `v4l2-ctl --device <node> --list-formats-ext` for discrete pixel format/resolution capabilities; enriches from `/sys/class/video4linux/video*/device/uevent` for kernel driver and from adjacent USB sysfs fields for VID/PID, manufacturer, product, serial, bus/device, interface, and class; falls back to `/sys/class/video4linux/video*` for basic name, node, driver, and available USB identity when `v4l2-ctl` cannot run. | Basic video device discovery, Deepin-style physical-device deduplication, Linux video4linux sysfs fallback, lightweight driver enrichment, USB identity enrichment, and V4L2 format/resolution profile preservation. | No lshw/hwinfo fallback; no USB speed enrichment; format parsing intentionally covers discrete size entries. | P2 |
...
- Input, printer, and CD-ROM still have limited fallback/enrichment coverage compared with the reference projects; audio still lacks hwinfo/profile enrichment; camera still lacks lshw/hwinfo and USB speed enrichment.

...
- Network probing uses `ip -j link` plus optional `ip -j addr`, enriches interfaces from `/sys/class/net/*` with speed, duplex, wireless/ethernet capability marking, and kernel driver, and falls back to sysfs interfaces when `ip -j link` cannot run.
...
- USB probing enriches `lsusb` records from `/sys/bus/usb/devices/*` when bus/device numbers match, including max power, and falls back to sysfs devices when `lsusb` cannot run.
...
- CD-ROM probing enriches `/proc/sys/dev/cdrom/info` drives from `/sys/class/block/sr*` with basic vendor/model/serial identity and falls back to sysfs drives when proc cdrom info cannot be read or parses no drives.
...
- Network probing now preserves IPv4/IPv6 addresses from `ip -j addr` when available; `/sys/class/net` preserves interface name, MAC, operstate, speed, duplex, wireless/ethernet capabilities, and kernel driver, including when `ip -j link` cannot run.
- Network probing now marks non-wireless physical interfaces with an `ethernet` capability when sysfs exposes driver, speed, or duplex evidence.
...
- CD-ROM probing now preserves sysfs vendor/model/serial identity on proc-discovered drives as well as sysfs fallback drives.
...
| Network | `ip -j link`, interface/MAC/operstate, optional `ip -j addr` IPv4/IPv6 enrichment; filters loopback/common virtual interfaces; malformed JSON produces warning; enriches interfaces from `/sys/class/net/*` with speed, duplex, wireless/ethernet capabilities, and `DRIVER=` from uevent; falls back to sysfs interfaces when `ip` cannot run. | Basic network interface enumeration, IP address enrichment, Kylin-style avoidance of non-physical interfaces, Linux sysfs fallback, and lightweight sysfs driver plus wireless/ethernet capability enrichment. | No lshw/lspci/NM DBus fallback; no dedicated ethernet/wireless type field beyond capabilities. | P1/P2 |
| Audio | `/proc/asound/cards`, card index/name; enriches from `/proc/asound/card*/codec#*` for codec and `/sys/class/sound/card*/device` for driver, normalized vendor, and subsystem IDs; falls back to `/sys/class/sound/card*` for ALSA card index/name plus available enrichment when proc asound cards is unavailable. | Deepin/Kylin use `/proc/asound` and multimedia sources; base source, Linux sysfs card fallback, lightweight codec, driver, vendor, and subsystem enrichment absorbed. | No lshw/hwinfo/PCI fusion; limited profile data; vendor alias table remains intentionally small. | P2 |
...
| CD-ROM | `/proc/sys/dev/cdrom/info`, drive names and basic capabilities; enriches proc-discovered drives from `/sys/class/block/sr*` with vendor/model/serial identity; falls back to sysfs drive nodes plus identity when proc cdrom info is unavailable or parses empty. | Proc cdrom discovery, Linux sysfs block enrichment/fallback, basic sysfs identity preservation, and skip-empty warning behavior. | No lshw/hwinfo/lsscsi fallback; no firmware enrichment; sysfs fallback cannot recover capabilities. | P2 |
| USB | `lsusb` for bus/device/VID/PID/product plus matched `/sys/bus/usb/devices/*` enrichment for class/subclass/protocol, manufacturer, serial, speed, and max power; falls back to sysfs for bus/device IDs, VID/PID, identity, class data, speed, and max power when `lsusb` cannot run; filters root hubs, USB hubs, sysfs host controllers, and sysfs interface entries. | Basic USB enumeration, Linux sysfs success-path enrichment and fallback including max power, and Deepin/Kylin hub filtering. | No `lsusb -v`; no detailed interface descriptor enrichment; USB devices consumed by Bluetooth/camera/input/printer are not deduplicated. | P2 |
...
- Audio preserves the missing/failed `/proc/asound/cards` warning while still emitting ALSA card devices from `/sys/class/sound/card*` when present, with sysfs driver/vendor/subsystem enrichment where available.

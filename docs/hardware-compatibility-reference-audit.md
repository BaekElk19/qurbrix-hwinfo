...
Four confirmed defects were fixed during this audit:
...
| BIOS / motherboard / DMI | `dmidecode -t 0,1,2,3`, BIOS vendor/version/date and baseboard manufacturer/product/serial. | Core DMI BIOS/baseboard parsing. | No `/sys/class/dmi/id` fallback; success with empty DMI output can still create generic BIOS/Motherboard devices; chassis/system/language/memory-array data not modeled. | P2 |
...
| Network | `ip -j link`, interface/MAC/operstate. | Basic network interface enumeration. | No lshw/lspci/sysfs/NM DBus fallback; no loopback/virtual filtering; no driver/wireless/type/speed/duplex/IP enrichment; malformed JSON becomes empty output without warning. | P0/P1 |
...
| Camera | `v4l2-ctl --list-devices`, emits one device per `/dev/video*`. | Basic video device discovery. | No sysfs/lshw/hwinfo fallback; no vendor/driver/speed/serial; UVC multi-node cameras can duplicate physical devices. | P0/P2 |
| Battery | `upower --dump`, battery capacity/energy/voltage/vendor/model/serial. | UPower-based collection. | No line-power filtering; no sysfs cycle/temp fallback. | P0/P2 |
...
| USB | `lsusb`, bus/device/VID/PID/product. | Basic USB enumeration. | No `lsusb -v`; no interface/class/speed/maxpower/serial enrichment; root hubs and hubs are not filtered; USB devices consumed by Bluetooth/camera/input/printer are not deduplicated. | P0/P2 |
...
- P0: filter obvious false positives: loopback/virtual network interfaces, `upower` line-power devices, USB root hubs/hubs, empty BIOS/DMI records, duplicate UVC camera nodes.

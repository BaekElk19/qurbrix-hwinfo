...
| Battery | `upower --dump`, battery capacity/energy/voltage/vendor/model/serial; filters line-power devices; falls back to `/sys/class/power_supply/BAT*` for battery fields including cycle count; normalizes `LGC` to `LG Chem`. | UPower-based collection, Deepin-style line-power filtering, Linux sysfs battery fallback, and lightweight battery vendor aliasing. | No temperature fallback; vendor alias table remains intentionally small. | P2 |

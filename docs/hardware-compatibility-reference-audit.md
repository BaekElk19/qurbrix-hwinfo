...
- Network probing uses `ip -j link` plus optional `ip -j addr`, enriches interfaces from `/sys/class/net/*` with speed, duplex, wireless/ethernet type/capability marking, and kernel driver, and falls back to sysfs interfaces when `ip -j link` cannot run.
...
| GPU | `lspci -nn -k` display/VGA/3D records, driver/modules, text and numeric vendor normalization; falls back to display-class `/sys/bus/pci/devices/*` records with kernel driver and module data when lspci cannot run. | PCI GPU parsing and domestic GPU aliases from Kylin; driver/module extraction from lspci; Linux sysfs PCI display-class fallback with lightweight driver and module preservation. | No `lshw -C display`, `glxinfo -B`, `/sys/class/drm/card*/device`, dmesg/nvidia memory enrichment; sysfs fallback lacks human-readable device names. | P2/P3 |
...
| Network | `ip -j link`, interface/MAC/operstate, optional `ip -j addr` IPv4/IPv6 enrichment; filters loopback/common virtual interfaces; malformed JSON produces warning; enriches interfaces from `/sys/class/net/*` with speed, duplex, wireless/ethernet `network_type` and capabilities, and `DRIVER=` from uevent; falls back to sysfs interfaces when `ip` cannot run. | Basic network interface enumeration, IP address enrichment, Kylin-style avoidance of non-physical interfaces, Linux sysfs fallback, and lightweight sysfs driver plus wireless/ethernet type/capability enrichment. | No lshw/lspci/NM DBus fallback. | P1/P2 |
...
| PCI / Other PCI | `lspci -nn -k`, class/vendor/device IDs, driver/modules; falls back to `/sys/bus/pci/devices/*` for address, vendor/device/class/subsystem IDs, kernel driver, and module names when lspci cannot run; sysfs class IDs retain the full 24-bit code such as `040300`; display-class sysfs PCI nodes are consumed by GPU; unconsumed PCI devices become `OtherPci`. | PCI class and driver/module extraction plus Linux sysfs PCI ID, driver, and module fallback; GPU consumption of sysfs display-class PCI nodes. | Network/audio/storage/camera/bluetooth may duplicate as `OtherPci`; sysfs fallback lacks human-readable vendor/device/class names. | P2 |

...
- Monitor uses `xrandr --query`, `xrandr --verbose`, and `/sys/class/drm/*/edid`, parses EDID in-process, preserves identity/date/size/preferred mode/gamma/diagonal inches and xrandr max mode, and keeps monitor devices when EDID parsing fails.
- GPU uses `lspci -nn -k`, normalizes common and domestic GPU vendor names, falls back to PCI vendor IDs when lspci text is generic, optionally enriches matched PCI adapters from `lshw -class display`, enriches matched DRM adapters with VRAM totals from `/sys/class/drm/*/device/mem_info_vram_total`, and consumes display-class `/sys/bus/pci/devices/*` nodes with kernel driver data when `lspci` cannot run.
...
- BIOS/motherboard probing falls back to `/sys/class/dmi/id` when `dmidecode` is missing or denied, and BIOS probing now records Linux firmware type plus Secure Boot state from `/sys/firmware/efi` when available.
...
- Storage probing requests WWN/firmware revision from `lsblk`, enriches kernel driver data from `/sys/block/*/device/uevent`, optionally enriches human-readable disk identity from `lshw -class disk`, optionally enriches SMART health, temperature, and NVMe health counters from `smartctl -a -j`, and falls back to `/sys/block/*` when `lsblk` cannot run.
...
| Monitor | `xrandr --query` for connector/resolution and first advertised mode as max resolution, `xrandr --verbose` and sysfs EDID for identity; EDID parser fills manufacturer/product/date/size/preferred mode/gamma and probe computes diagonal inches. | Kylin xrandr verbose EDID extraction including max mode; Deepin sysfs DRM EDID parsing pattern; no `/tmp` temp file or external `edid-decode`. | No `hwinfo_monitor`; ambiguous duplicate sysfs connectors are skipped rather than matched by card. | P2 |
| GPU | `lspci -nn -k` display/VGA/3D records, driver/modules, text and numeric vendor normalization; optionally enriches matched PCI adapters from `lshw -class display`; optionally enriches matched DRM adapters with VRAM totals from `/sys/class/drm/*/device/mem_info_vram_total`; falls back to display-class `/sys/bus/pci/devices/*` records with kernel driver and module data when lspci cannot run, with lshw and DRM enrichment when available. | PCI GPU parsing and domestic GPU aliases from Kylin; Kylin-style `lshw -C display` identity source; driver/module extraction from lspci; Linux sysfs PCI display-class fallback with lightweight driver/module preservation and DRM VRAM enrichment. | No `glxinfo -B`, dmesg/nvidia memory/render enrichment; lshw/DRM parsing intentionally covers PCI-address-matched display records and common sysfs fields. | P2/P3 |
...
| BIOS / motherboard / DMI | `dmidecode -t 0,1,2,3`, with `/sys/class/dmi/id` fallback for BIOS vendor/version/date and baseboard manufacturer/product/serial when dmidecode cannot run; enriches BIOS with firmware type and Secure Boot state from `/sys/firmware/efi`; empty dmidecode output still produces `source_empty` rather than generic devices. | Core DMI BIOS/baseboard parsing, sysfs DMI fallback, Linux firmware type/Secure Boot enrichment, and Deepin-style skip-empty behavior. | Chassis/system/language/memory-array data not modeled. | P2 |
| Storage | `lsblk -J -b -o NAME,TYPE,SIZE,MODEL,SERIAL,TRAN,WWN,REV`, disk-only filtering, parse failure warning, WWN-backed stable IDs, firmware revision; enriches kernel driver from `/sys/block/*/device/uevent`; optionally enriches matched device nodes from `lshw -class disk` with human-readable vendor/model/serial/firmware; optionally reads `smartctl -a -j <device>` for SMART health, temperature, and NVMe health counters, including non-zero smartctl JSON; falls back to `/sys/block/*` for node/vendor/model/serial/WWN/firmware/size/rotational media type and driver when `lsblk` cannot run. | Basic lsblk disk enumeration, Linux sysfs disk fallback with WWN/firmware and lightweight driver preservation, lshw disk identity enrichment, plus smartctl SMART/temp/NVMe health-counter enrichment with non-zero-status JSON handling. | No hwinfo/hdparm fusion; no controller enrichment. | P2 |
...
- P2: add network NM DBus enrichment, storage controller/hwinfo/hdparm enrichment, richer multi-interface USB modeling, camera hwinfo enrichment, broader audio SoC alias coverage, and Bluetooth hwinfo/DBus enrichments.
